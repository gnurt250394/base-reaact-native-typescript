  ...
  async request<T>(
    ...
    const requestInterceptor = instance.interceptors.request.use(
      config => {
        ...
        const {user, position}: AuthReducer = store.getState().userProfile;
        if (user.loginToken && user.loginToken.length > 0) {
          configTemp.headers.Authorization = user.loginToken;
        }
        configTemp.headers.role = position;
        // Add some header at here
