...
export const useKeyboardShowTranslation = (doneButtonHeight?: number) => {
  ...
  useEffect(() => {
    // @ts-ignore
    let keyboardHide = Keyboard.addListener("keyboard" + action + "Hide", _keyboardWillHide);
    // @ts-ignore
    let keyboardShow = Keyboard.addListener("keyboard" + action + "Show", _keyboardWillShow);
    // cleanup function
    return () => {
