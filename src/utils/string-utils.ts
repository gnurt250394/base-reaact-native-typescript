...
Date.prototype.getAge = function () {
  var date = new Date();
  if (date.getFullYear() < this.getFullYear()) return 0;
  if (date.getFullYear() === this.getFullYear()) return 1;
  return date.getFullYear() - this.getFullYear();
};

Date.prototype.compareDate = function (date) {
  if (this.ddmmyyyy() == date.ddmmyyyy()) return 0;
...
Number.prototype.compareDate = function (timeMilisecond) {
  ...
  if (date.ddmmyyyy() == date1.ddmmyyyy()) return 0;
  ...
  function (this: Date) {
    for (let i = 31; i > 0; i--) {
      try {
        ...
        if (date.isValidDate() && date.getDate() == i) return date;
      } catch (error) {}
    }
    return this;
  };
